...
<div class="dxrd-surface" data-bind="styleunit: { 'width': Math.min($data.pageWidth() + $data.maxMarkerWidth() + $root.verticalScrollOffset() , $root.surfaceSize()) }, css: { 'dxrd-surface-rtl': rtl() }, click: function(surface, e) { $root.selection.clickHandler($root.selection.dropTarget ? null : surface, e); e.stopPropagation(); }, keyDownActions: $root.actionLists">
...
<div class="dxrd-table-control" data-bind="style: { visibility: (selected() || focused()) ? 'inherit' : 'hidden'}, css: { 'dxrd-selected': selected, 'dxrd-focused': focused, 'dxrd-intersect': isIntersect, 'dxrd-locked': locked }, resizable: $root.resizeHandler, draggable: $root.dragHandler, styleunit: position">
    ...
    <div class="dxrd-table-column-selection dxrd-table-column-selection-top" data-bind="click: function(_, e) { selectColumn($root.selection); e.stopPropagation(); }, visible: !$root.dx.Analytics.Internal.DragDropHandler.started(), styleunit: { 'left': position.left, 'width': position.width }"></div>
    <!-- /ko -->
    <!-- ko foreach: rows -->
    <div class="dxrd-table-row-control" data-bind="styleunit: { 'top': position.top, 'height': position.height }">
    ...
    <div class="dxrd-table-column-selection dxrd-table-column-selection-bottom" data-bind="click: function(_, e) { selectColumn($root.selection); e.stopPropagation(); },visible: !$root.dx.Analytics.Internal.DragDropHandler.started(), styleunit: { 'left': position.left, 'width': position.width }"></div>
    <!-- /ko -->
    <!-- /ko -->
</div>
...
<div class="dxrd-table-control" data-bind="visible: isSelected() && !(selected() || focused()), styleunit: position">
    ...
    <!-- /ko -->
    <!-- ko foreach: rows()[rows().length-1].cells -->
    <div class="dxrd-table-column-selection dxrd-table-column-selection-bottom" data-bind="visible: !$root.dx.Analytics.Internal.DragDropHandler.started(),click: function(_, e) { selectColumn($root.selection); e.stopPropagation(); }, styleunit: { 'left': position.left, 'width': position.width }"></div>
    <!-- /ko -->
    <!-- /ko -->
</div>
...
<script type="text/html" id="dxrd-menubutton-template">
    <div class="dxrd-menubutton-part" tabindex="1" style="outline:inherit" data-bind="cacheElement: { action: function(element){ $data.setMenuElement(element) } }">
        <div class="dxrd-menu-container dxrd-menu-fullscreen-nav" data-bind="css: { 'dxrd-menu-fullscreen-nav-collapsed': isMenuCollapsed()}">
            <div class="dx-overlay-wrapper dxd-menu-back-color dxd-back-contrast" data-bind="styleunit: { width: $data.isMenuCollapsed() ? 50 : 300 }, visible: appMenuVisible">
...
<script type="text/html" id="dxrd-reportSourceUrl">
    <div data-bind="dxSelectBox: { items: getItems($root.reportUrls, $root.navigateByReports.currentTab), value: value, disabled: disabled, valueExpr: 'Key', displayExpr: 'Value', dropDownOptions: { container: $root.getPopupContainer($element) } }"></div>
</script>
